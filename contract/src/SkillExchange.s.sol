...
contract SkillExchange is ERC721, Ownable {
    ...
    enum RequestStatus { Pending, Accepted, Rejected, InProgress, Completed, Disputed }
    enum AgreementStatus { Active, Completed, Disputed, Canceled }
    ...
    constructor(IERC20 _token) ERC721("Skill Completion Badge", "SKILL") {
        token = _token;
    }

    function createListing(string memory _skillName, string memory _description) public {
        listings[listingCounter] = SkillListing(
            listingCounter,
            msg.sender,
            _skillName,
    ...
    function requestService(uint256 _listingId, string memory _description, uint256 _deadline) public {
        require(listings[_listingId].isAvailable, "Listing not available");
        require(listings[_listingId].userAddress != msg.sender, "Cannot request own service");
    ...
    function respondToRequest(uint256 _requestId, bool _accept) public {
        ...
        require(listings[request.listingId].userAddress == msg.sender, "Not the service provider");
    ...
    function createAgreement(uint256 _requestId) private {
        ...
        emit AgreementCreated(agreementCounter, listings[request.listingId].userAddress, request.requester);
        agreementCounter++;
    }
    ...
    function resolveDispute(uint256 _disputeId, bool _favorProvider) public onlyOwner {
    ...
    function getUserReputation(address user) public view returns (
        ...
        return (rep.positivePoints, rep.negativePoints, rep.positivePoints + rep.negativePoints);
    }
}